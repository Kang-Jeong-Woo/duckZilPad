...
import { imgDataState } from "@/store/slices/img-slice";
import { drawData } from "@/store/slices/canvas-slice";
    ...
    if (req.method === "POST") {
        ...
        try {
            ...
            if(userData) {
                ...
                const drawData: drawData = req.body.drawData;
                ...
                if(imgData.length > 0) {
                    ...
                    for (let i = 0; i < imgData.length; i++) {
                        ...
                        // 업데이트
                            ...
                            await ImgData.updateOne({_id: new ObjectId(imgData[i]._id)}, {$set : imgData[i]})
                        // 신규생성
                        } else {
                            const { _id, ...others } = imgData[i]
